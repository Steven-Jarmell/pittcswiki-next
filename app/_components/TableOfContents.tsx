...
export default function TableOfContents({ headings }: TOCT) {
  const [activeHeading, updateActiveHeading] = useState(
    headings.length > 0 ? headings[0] : null
  )

  const checkForNewActiveHeading = throttle(
    (e: any) => {
  ...
  return (
    <ul className="hidden lg:block fixed lg:left-0 list-none ml-0 markdown-toc">
      {headings.map((heading: any) => (
        ...
        >
          <a className="text-gray-600" href={"#" + slugger.slug(heading.value)}>
